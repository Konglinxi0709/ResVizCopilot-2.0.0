    ...
    def _element_to_dict(self, element: ET.Element) -> Union[Dict[str, Any], str, None]:
        ...
        if len(element) > 0:
            ...
            if element.text and element.text.strip():
                # 还原文本中的转义字符
                result['_text'] = self._unescape_string(element.text.strip())
            
            # 添加属性（同时还原属性值中的转义字符）
            if element.attrib:
                unescaped_attribs = {
                    k: self._unescape_string(v) for k, v in element.attrib.items()
                }
                result['_attributes'] = unescaped_attribs
