...
1. 每个非继承的子研究问题需要包含以下内容
    1. 名称：用简洁易懂的语言清晰的描述这一问题的主要任务，名称应该是问句形式，例如是否...，如何...。除了inherit类型的问题以外，任何子问题的名称不能与当前研究中存在的任何节点名称相同，包括你当前研究方案中的问题名称。这意味着即便你只是希望微调某个问题的内容，你也必须对其标题做出修改，以确保名称不重复。如果实在不适宜修改标题，你可以在标题后添加"(1)"、"(2)"、"(3)"等后缀以作区分。
