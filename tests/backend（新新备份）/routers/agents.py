...
from backend.shared_manager import shared_database_manager, shared_project_manager
...
auto_research_agent = AutoResearchAgent(
    ...
    publish_callback=shared_project_manager.publish_patch,
    database_manager=shared_database_manager,
    get_visible_messages=shared_project_manager.get_visible_messages
)
...
shared_project_manager.register_agent("auto_research_agent", auto_research_agent)
shared_project_manager.register_agent("user_chat_agent", user_chat_agent)
...
async def sse_send_message(request: SendMessageRequest):
    ...
    # 获取指定的智能体
    agent = shared_project_manager.get_agent(request.agent_name)
    if not agent:
        raise HTTPException(
            status_code=400, 
    ...
    async def event_stream():
        ...
        try:
            ...
            # 订阅消息更新
            async for patch in shared_project_manager.subscribe_patches():
                # 检查智能体是否还在处理
...
async def get_message_history():
    ...
    Returns:
        消息历史响应
    """
    messages = shared_project_manager.get_message_history()
    incomplete_msg = shared_project_manager.get_incomplete_message()
...
async def sse_continue_message(message_id: str):
    ...
    # 检查消息是否存在
    message = shared_project_manager.get_message(message_id)
    if not message:
...
async def stop_generation():
    ...
    for agent_name, agent in shared_project_manager._agents.items():
        if agent.is_processing():
    ...
    if stopped_agents:
        ...
        shared_project_manager.log_message_history()
        return StopResponse(
            status="success", 
...
async def get_agent_status():
    ...
    Returns:
        智能体状态信息
    """
    status = shared_project_manager.get_status()
...
def _get_snapshot_object(snapshot_id: str) -> Dict[str, Any]:
    ...
    """
    if snapshot_id:
        return shared_project_manager.get_database_snapshot(snapshot_id)
    return None
