...
`
    const countryLines = countryFile.split('\n');
    countryLines.forEach(line => {
        const names = line.trim().split(/\s+/);
        if (names.length >= 2) {
            trans[names[1]] = names[names.length - 1];
        }
    });

    const country2num = new Map();
    data.forEach(paper => {
        paper.authors.forEach(author => {
            if (author.country in trans) {
                const country = trans[author.country];
                country2num.set(country, (country2num.get(country) || 0) + 1);
            }
        });
    });

    const dataOutput = [];
    country2num.forEach((num, country) => {
        dataOutput.push({ country, num });
    });
