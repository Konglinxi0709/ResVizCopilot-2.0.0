...
export class QuestionClassificationTool extends StructuredTool {
    constructor(llm) {
        ...
        Provide the output in the following format and do not include anything beyond the requirements, including explanations:
        Matching Status[string]: choose one from {Template-based, Non-Template-based}
        Matching Template[string]: if matched, output the corresponding template's name, otherwise output 'None';
        Modify need[string]: output 'Yes' if question's parameters are same values with variants' default, otherwise output 'No'`;
    }
...
Output:
{
    ...
    async run(inputs) {
        ...
        try {
            ...
            const {Status, "Final Question": finalQuestion} = result;
