...
class ProjectManager:
    ...
    def __init__(self, data_manager: Optional[DataManager] = None):
        ...
        self.data_manager = data_manager or DataManager()  # 数据管理器
    ...
    async def _create_message_from_patch(self, patch: Patch) -> str:
        ...
        # 创建新消息
        message = Message(
            role="assistant",  # 默认为智能体消息
            status="generating"
        )
        
        # 如果补丁包含用户内容，设置为用户消息
        if patch.content_delta and not patch.thinking_delta:
            message.role = "user"
    ...
    async def _process_patch_for_frontend(self, patch: Patch) -> FrontendPatch:
        ...
        if patch.snapshot_id:
            snapshot_obj = self.data_manager.get_snapshot(patch.snapshot_id)
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "database_state": self.data_manager.get_database_state()
        }
    ...
    def execute_database_action(self, action_type: str, params: Dict) -> Dict:
        """执行数据库操作"""
        return self.data_manager.execute_action(action_type, params)
    
    def get_database_snapshot(self, snapshot_id: str) -> Optional[Dict]:
        """获取数据库快照"""
        return self.data_manager.get_snapshot(snapshot_id)
