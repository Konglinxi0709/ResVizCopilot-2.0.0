...
**✅ 当前阶段实现的接口：**
- `GET /research-tree/snapshots/{snapshot_id}` - 获取指定快照的完整数据
- `GET /research-tree/snapshots/current-id` - 获取当前研究树快照的唯一ID`

**🚧 后续版本接口（暂不实现）：**
- `POST /research-tree/problems/root` - 创建新的根级研究问题
- `PATCH /research-tree/problems/root/{problem_id}` - 更新根问题的标题、意义、标准等
- `DELETE /research-tree/problems/root/{problem_id}` - 删除指定的根问题
- `POST /research-tree/problems/{problem_id}/solutions` - 为问题创建解决方案
- `PATCH /research-tree/solutions/{solution_id}` - 更新解决方案内容
- `DELETE /research-tree/solutions/{solution_id}` - 删除解决方案及其子树
- `POST /research-tree/problems/{problem_id}/selected-solution` - 设置问题的当前选中解决方案
